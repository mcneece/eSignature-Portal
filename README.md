...
<p>Adobe Acrobat Sign has many different features and functionalities to match a user's requirements for sending out a document for signature, but not all can be done from the browser solution. Some require development skills and knowledge of making API calls to Adobe for fetching information or making bulk requests. With the release of the eSignature Support Portal this can all be done by a click of a button and zero development skills.</p>
<ul>This includes:
    <li>Automating the flow of information for requesting access to Adobe Acrobat Sign using a handful of API calls.</li>
...
<p>In addition to simplifying functionality for the common user, the eSignature Support Portal also is the hub for: curated training material for novice to expert Acrobat Sign users, the fastest routes to submitting service or incident requests to the eSignature Operations Team, and links to the Adobe Sign tool.</p>
...
```python
    class Config(object):
        ...
        SECRET_KEY = # Insert PRODUCTION Access Token w: USER READ privileges
...
```
<h3>Step 3: Run Commands in Terminal to Run Flask Project</h3>
